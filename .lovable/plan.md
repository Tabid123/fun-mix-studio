...
Dhammaan tallaabooyinka (PIN iyo non-PIN, dhammaan shirkadaha) waxay marayaan **isku dariiq mid ah**:

```text
Dialog cusub la arkay
   -> match step (keyword ama sequential fallback)   [sida hadda]
...
4. **Hubin ka nuglaan yar (Somtel/Amtel fix)**: `isValueCommittedInActiveField` waxaa lagu darayaa xaalad saddexaad — haddii field-ku yahay password/masked oo `text` madhan yahay laakiin `findMaskedPinLengthInTree` = dhererka la filayo, waa la aqbalayaa. Sidoo kale, ka dib 2 dib-u-qorid, haddii field-ku leeyahay `textLength > 0` (xog ku jirta) Send **waa la riixayaa** — halkii laga joojin lahaa gebi ahaanba. Tani waa arinta keentay "Send ma taabanaayo" ee Somtel iyo tallaabada PIN-ka ee Amtel.
